...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  hid_output = logistic(hid_input);               % REQD
  ...
  class_prob = exp(log_class_prob);               % REQD
  ...
  A = model.hid_to_class';
  B = (class_prob - data.targets);
  %A = a * b;
  c = hid_output;
  c1 = (1 - hid_output);
  C = (c .* c1);          % DOT PRODUCT (SCALAR)
  %B = A * c;
  D = data.inputs';
  B1 = (A * B);
  C1 = (B1 .* C);         % DOT PRODUCT (SCALAR)
  D1 = C1 * D;
  wih_decay = wd_coefficient * model.input_to_hid;
...
%   ret.hid_to_class = model.hid_to_class * wd_coefficient;
  ret.input_to_hid = (D1 / size(data.targets, 2)) + wih_decay;
  E = ret.input_to_hid;
%  ret.input_to_hid = (class_prob - data.targets)' * model.hid_to_class

  P = B * c';
  whs_decay = wd_coefficient * model.hid_to_class;
  ret.hid_to_class = (P / size(data.targets, 2)) + whs_decay;
  F = ret.hid_to_class;
