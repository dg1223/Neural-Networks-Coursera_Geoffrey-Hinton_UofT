...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  [row_itoh, col_itoh] = size(model.input_to_hid);
  n_param_itoh = row_itoh * col_itoh;
  [row_htoc, col_htoc] = size(model.hid_to_class);
  n_param_ihtoc = row_htoc*col_htoc;
  
  original_weights = model_to_theta(model);
  decayed_weights = wd_coefficient*original_weights;
  
  weights_to_hid = reshape(decayed_weights(1:n_param_itoh,:),row_itoh,col_itoh);
  weights_to_class = reshape(decayed_weights((n_param_itoh+1):end,:),row_htoc,col_htoc);
  
  ret.input_to_hid = model.input_to_hid .* weights_to_hid;
  ret.hid_to_class = model.hid_to_class .* weights_to_class;
end
